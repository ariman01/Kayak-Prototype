...
class UserFlightHistory extends Component {
...
getUserHistoryTile(userhistory){
  return userhistory.map((history)=>{
    return(
      ...
      <td>{history.flight_name}</td>
      <td>{history.src_city}</td>
      <td>{history.destination_city}</td>
      <td>{history.booking_date.slice(0,10)}</td>
            ...
            <div>
              ...
              <div className= "user-profile-body">
                  <div className= "user-profile-body-nav-options">
                    ...
                    <p style={{fontSize : "20pt"}}>User Flight Booking History</p>
                    <br/><br></br>
                    <br></br>
                    <div><table className="table table-hover table-condensed" style={{width:700}}>
                    <thead>
                    </thead>
...
function mapStateToProps(state) {
    console.log("state.users.userhistory",state.users.userflighthistory);
    return{
        userhistory:state.users.userflighthistory
    }
}
...
export default connect(mapStateToProps,mapDispatchToProps)(UserFlightHistory);
